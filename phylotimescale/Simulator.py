...
class Simulator:
    ...
    def batch_mrbayes(self):
        ...
        for idx in self.data.index:
            ...
            # Define mrbayes object with ipa.
            mb = ipa.mrbayes(
            ...
            propset ParsSPRClock(Tau,V)$prob=0;\n\n''')
                ...
                for i in range(len(self.mb_params)):
                    if i == len(self.mb_params) - 1:
                        contents.insert((25 + i), "  " 
                        + calib_string.format(self.mb_params[i][0], self.mb_params[i][1], self.mb_params[i][2]))
                        fd.seek(0)
                    else: 
                        contents.insert((25 + i), 
                        "  " + calib_string.format(self.mb_params[i][0], self.mb_params[i][1], self.mb_params[i][2]) + "\n\n")
            ...
            # Run mrbayes object.
            subprocess.run(
                    ["mb", mb.nexus], # Add relative conda path.
                    check=True,
                    stdout=subprocess.PIPE,
                )
